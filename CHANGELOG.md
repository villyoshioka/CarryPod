...
- **サイトマップの最終更新日**: サイトマップの投稿・固定ページに最終更新日（lastmod）を付けるようにしました。トップページにはサイト内で最も新しい投稿の更新日が付きます。
- **サイトマップの対象ページ**: noindex が指定されているページと RSS フィードを、サイトマップに含めないようにしました。Mati で「検索エンジンのインデックスを拒否」を有効にしている場合、サイトマップにはページが含まれなくなります。

### バグ修正

- 除外パターンで除外したページが、サイトマップと llms.txt に含まれてしまう問題を修正しました。
...
- **ヘッダー変換ルールの案内**: Cloudflare 連携時に表示される「レスポンス ヘッダー変換ルールの設定」の案内に「非表示にする」を追加しました。Cloudflare への設定が済んだら案内を隠しておけます。Mati の設定変更などで設定する内容が変わったときや、Wrangler を使用しない設定に切り替えたときは、案内が再び表示されます。
...
- **llms.txtの形式準拠**: llms.txtを [llmstxt.org](https://llmstxt.org) の形式に準拠させました。これまでのアクセスポリシー（AI学習への利用禁止など）はそのまま維持し、サイト名・サイトの説明・ページ一覧を含む形式で出力するようになりました。

### バグ修正

- URL形式が「相対パス」の場合に、robots.txtのサイトマップURLが無効と判定される問題を修正しました。
...
- **バックグラウンド処理ライブラリの更新**: バックグラウンド処理に使用している Action Scheduler を 4.2.0 に更新しました。
...
- カテゴリ・タグ・著者などのアーカイブで、特定の条件下で `page/2/` 以降が生成されず404になる問題を修正しました。
...
1. [Releases](https://github.com/villyoshioka/CarryPod/releases) から v3.0.0 の ZIP ファイルをダウンロード
2. WordPress管理画面 → プラグイン → Carry Pod を無効化（推奨）  
...
- **Mati v1.7.0連携対応**: Matiで設定したiframe埋め込み許可ドメインが`_headers`ファイルに反映されるようになりました。また、Blueskyドメイン認証用のファイルを自動生成するようになりました。
